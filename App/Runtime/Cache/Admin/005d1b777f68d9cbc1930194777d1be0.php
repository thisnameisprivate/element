...
<script type="text/javascript">
    layui.use(['table', 'layer', 'form'], () => {
        ...
        var tableIns = table.render({
            ...
            elem: '#container',
            toolbar: '#toolbaradd',
            url: "<?php echo U('Admin/Index/diseaseCheck');?>",
            height: 'full-200',
            cellMinWidth: 150,
            size: 'sm',
            cols: [[
        ...
        table.on('tool(edittable)', obj => { // tool delete
            ...
            if (layEvent == 'del') {
                ...
                layer.confirm('【'+ data.hospital +'】are you soure delete ?', index => {
                    ...
                    client.open("GET", "<?php echo U('Admin/Index/diseaseDel/id/"+ parseInt(data.id) +"');?>");
                    client.send();
        ...
        table.on('toolbar(edittable)', obj => {
            if (obj.event === 'add') { // add data
                ...
                form.on('submit(formSubmit)', data => {
                    var client = new XMLHttpRequest();
                    client.open('GET', "<?php echo U('Admin/Index/diseaseAdd/data/"+ JSON.stringify(data.field) +"');?>");
                    client.send();
                    client.onreadystatechange = () => {
                        if (client.readyState === 4 && client.status === 200) {
                            if (client.response == 1) {
