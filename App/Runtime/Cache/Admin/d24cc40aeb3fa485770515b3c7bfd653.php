...
<head>
    ...
    <style>
        .container{position:relative; width:500px; height:200px; left:0; right:0; bottom:0; top:0; margin:auto; padding-top:20px;}
        ...
        <form class="layui-form">
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="fromaddress" required lay-verify="required" placeholder="请输入新的医院科室" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
...
<script type="text/javascript">
    layui.use(['table', 'layer', 'form'], () => {
        ...
        var tableIns = table.render({
            ...
            elem: '#container',
            toolbar: '#toolbaradd',
            url: "<?php echo U('Admin/Index/typesofCheck');?>",
            height: 'full-200',
            size: 'sm',
            cols: [[
                {field: 'id', title: 'N o.', width:80, sort: true},
                {field: 'fromaddress', title: '来源', width: 120},
        ...
        table.on('tool(edittable)', obj => {
            ...
            if (layEvent === 'del') {
                ...
                layer.confirm('【'+ data.fromaddress +'】 are you soure delete ?', index => {
                    ...
                    client.open('GET', "<?php echo U('Admin/Index/typesofDel/id/"+ parseInt(data.id) +"');?>");
                    client.send();
        ...
        table.on('toolbar(edittable)', obj => {
            ...
            form.on('submit(formSubmit)', data => {
                var client = new XMLHttpRequest();
                client.open("GET", "<?php echo U('Admin/Index/typesofAdd/data/"+ JSON.stringify(data.field) +"');?>");
                client.send();
                client.onreadystatechange = () => {
                    if (client.readyState === 4 && client.status === 200) {
                        if (client.response == 1) {
