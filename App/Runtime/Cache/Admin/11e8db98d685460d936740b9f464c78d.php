...
<script type="text/javascript">
    layui.use(['table', 'layer', 'form'], () => {
        ...
        var tableIns = table.render({
            ...
            elem: '#container',
            toolbar: '#toolbaradd',
            url: "<?php echo U('Admin/Index/arrivalStatusCheck');?>",
            height: 'full-200',
            size: 'sm',
            cols: [[
        ...
        table.on('tool(edittable)', obj => {
            ...
            if (layEvent === 'del') {
                layer.confirm('【'+ data.fromaddress +'】 are you soure delete ?', index => {
                    ...
                    client.open('GET', "<?php echo U('Admin/Index/arrivalStatusDel/id/"+ parseInt(data.id) +"');?>");
                    client.send();
        ...
        */
        table.on('toolbar(edittable)', obj => {
            ...
            form.on('submit(formSubmit)', data => {
                var client = new XMLHttpRequest();
                client.open("GET", "<?php echo U('Admin/Index/arrivalStatusAdd/data/"+ JSON.stringify(data.field) +"');?>");
                client.send();
                client.onreadystatechange = () => {
                    if (client.readyState === 4 && client.status === 200) {
                        if (client.response == 1) {
